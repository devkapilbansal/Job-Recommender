...
def user_profile(Name):
    ...
    #return render_template("test.html",name=Name,det=d[0][2])
...
def job_details_recruiter(job_details):
    ...
    if request.method=='POST':
        if 'update' in request.form:
            ...
            state=request.form.get('state')
            address=request.form.get('address')
            #return redirect(url_for('test',det=det[0][6]))
...
def fjob_details(det,udet):
    ...
    if request.method=='POST':
        if 'apply' in request.form:
            t=dataQueries.job_apply(int(udet),det[0])
            #return redirect(url_for("test",t=t,det=type(det)))



    return render_template('apply_job.html',det=det)
    #return render_template('test.html',det=det)


@app.route("/job_applications/<rd>")
def job_applications(rd):
    rd=eval(rd)
    applicants=dataQueries.recruiter_job(rd[0][0])
    #return render_template('test.html',applicants=type(rd),rd=rd[0][0])
    return render_template('applications.html',applicants=applicants)

@app.route("/test/<det>",methods=["GET","POST"])
def test(det):
    return render_template("test.html",det=det)
