...
def fetch_job_id(Job_name):
    with sqlite3.connect("MainDatabase.sqlite") as conn:
        cur=conn.cursor()
        cur.execute('''SELECT Job_id,Job_name,Skill_required,Job_Description,Expected_salary,state_Name,Location,Company_Name FROM Jobs WHERE Job_id=?''',(Job_name,))
        row=cur.fetchall()
        return row[0]
def fetch_recommend(State_Name,sector):
    with sqlite3.connect("MainDatabase.sqlite") as conn:
        cur=conn.cursor()
        cur.execute('''SELECT Job_name,State_Name,link FROM Jobs WHERE State_Name=? AND sector=?''',(State_Name,sector))
        row=cur.fetchall()
        return row
def recruiter_job(id_):
    with sqlite3.connect("MainDatabase.sqlite") as conn:
        cur=conn.cursor()
        cur.execute('''SELECT Job_id FROM Jobs WHERE Job_name=?''',(id_,))
        _id=cur.fetchone()[0]
        cur.execute('''SELECT User_id,Recruiter_id from Applications WHERE Job_id=?''',(_id,))
        rows=cur.fetchall()
        if len(rows)!=0:
